...
<div class="nhanhvn-admin-wrap">
  <h2><?php _e('Inventory Sync', 'nhanhvn'); ?></h2>
  <form method="post" class="tab-content">
    <?php wp_nonce_field('nhanhvn_sync_inventory'); ?>
    <p>
      <input type="submit" name="nhanhvn_sync_inventory" class="button-primary nhanhvn-sync-button"
        value="<?php _e('Sync All Inventory', 'nhanhvn'); ?>">
    </p>
    <table class="wp-list-table widefat fixed striped">
      ...
      <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                $log = $wpdb->get_row($wpdb->prepare(
                  "SELECT status, message FROM {$wpdb->prefix}nhanhvn_sync_log WHERE type = 'inventory_sync' AND item_id = %s ORDER BY sync_time DESC LIMIT 1",
                  $product->get_id()
                ));
                if ($log) {
        ...
        <?php else: ?>
          <tr>
            <td colspan="5"><?php _e('No products found.', 'nhanhvn'); ?></td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
    <p>
      <input type="submit" name="nhanhvn_sync_inventory" class="button-primary nhanhvn-sync-button"
        value="<?php _e('Sync Selected Inventory', 'nhanhvn'); ?>">
    </p>
  </form>
</div>
